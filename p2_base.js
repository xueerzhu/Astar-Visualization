...
function rebuildWorld(key) {
  ...
  tile_height_step_main = window.p2_tileHeight ? window.p2_tileHeight() : 16;
...
function draw() {
  ...
  */
  ...
  for (let y = y0; y < y1; y++) {
    for (let x = x0; x < x1; x++) {
      drawTrail(tileRenderingOrder([x + world_offset.x, y - world_offset.y]), [
        camera_offset.x,
        camera_offset.y
      ]); // odd row
    }
...
function drawTile([world_x, world_y], [camera_x, camera_y]) {
  ...
  if (window.p2_drawTile) {
    window.p2_populateWorld(world_x, world_y, -screen_x, screen_y);
    window.p2_drawTile(world_x, world_y, -screen_x, screen_y);
  }
  translate(-(0 - screen_x), -screen_y);
}

function drawTrail([world_x, world_y], [camera_x, camera_y]) {
  let [screen_x, screen_y] = worldToScreen(
    [world_x, world_y],
    [camera_x, camera_y]
  );
  translate(0 - screen_x, screen_y);
  if (window.p2_drawTrail) {
    window.p2_drawTrail(world_x, world_y, -screen_x, screen_y);
  }
  translate(-(0 - screen_x), -screen_y);
}
